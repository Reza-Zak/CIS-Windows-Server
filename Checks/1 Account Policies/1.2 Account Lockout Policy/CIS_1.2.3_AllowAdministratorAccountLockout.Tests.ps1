<#
.SYNOPSIS
    CIS Check: 1.2.3 - Allow Administrator Account Lockout

.DESCRIPTION
    This test validates that the system is configured to allow the built-in 
    Administrator account to be locked out after failed logon attempts.
...
Describe "CIS Check: 1.2.3 - Allow Administrator Account Lockout" -Tag $script:testTags {
    Context "Administrator account lockout policy" {
        It "Should be set to Enabled (1)" {
            [int]$script:securityPolicy['AllowAdministratorLockout'] | Should -Be 1
        }
    }
}
