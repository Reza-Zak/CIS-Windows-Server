<#
.SYNOPSIS
    CIS Check: 1.1.5 - Password Complexity Requirement

.DESCRIPTION
    This test validates that the system is configured to enforce password complexity requirements.
...
Describe "CIS Check: 1.1.5 - Password Complexity Requirement" -Tag $script:testTags {
    Context "Password complexity policy" {
        It "Should be set to Enabled (1)" {
            [int]$script:securityPolicy['PasswordComplexity'] | Should -Be 1
        }
    }
}
