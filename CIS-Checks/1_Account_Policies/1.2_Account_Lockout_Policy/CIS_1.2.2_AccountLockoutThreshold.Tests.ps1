<#
.SYNOPSIS
    CIS Check: 1.2.2 - Account Lockout Threshold

.DESCRIPTION
    This test validates that the system is configured to lock out user accounts 
    after 5 or fewer invalid logon attempts, but not 0.
...
Describe "CIS Check: 1.2.2 - Account Lockout Threshold" -Tag $script:testTags {
    Context "Account lockout threshold policy" {
        It "Should be set to 5 or fewer invalid logon attempts, but not 0" {
            $value = [int]$script:securityPolicy['LockoutBadCount']
